/**
 * @file    app_main.c
 * @brief   UDP Multicast example for STM32F412 + W6300 SoM
 *
 * @details Joins a multicast group and receives/echoes data.
 *          Uses ioLibrary_Driver multicast.h API.
 *          Two modes available (uncomment one in the main loop):
 *            - multicast_recv     : receive only (default)
 *            - multicast_loopback : receive + echo back
 *
...
static uint8_t g_recv_buf[ETHERNET_BUF_MAX_SIZE] = {0};
...
/* ============================================================ */
/* Set multicast destination HW address (required for W6300)     */
/* Multicast MAC = 01:00:5E + lower 23 bits of multicast IP     */
/* ============================================================ */
static void multicast_set_dhar(uint8_t sn, uint8_t *multicast_ip)
{
...
void app_main(void)
{
    ...
    printf(" Multicast group : %d.%d.%d.%d:%d\r\n\r\n",
           g_multicast_ip[0], g_multicast_ip[1],
           g_multicast_ip[2], g_multicast_ip[3],
           g_multicast_port);

    /* ---- Set multicast MAC for W6300 ---- */
    multicast_set_dhar(SOCKET_MULTICAST, g_multicast_ip);
    ...
    while (1)
    {
        ...
        /* ============================================================ */
        /* Multicast — uncomment ONE mode to use                        */
        /* ============================================================ */

        /* Receive only (default) */
        multicast_recv(SOCKET_MULTICAST, g_recv_buf, g_multicast_ip, g_multicast_port);

        /* Receive + echo back */
        //multicast_loopback(SOCKET_MULTICAST, g_recv_buf, g_multicast_ip, g_multicast_port);
    }
}

#endif /* EXAMPLE_UDP_MULTICAST */
